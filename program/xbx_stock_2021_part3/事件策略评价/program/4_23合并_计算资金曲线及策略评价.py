...
# 新增策略评价：统计事件的频率（已经排除了那些因为涨跌停等原因无法买入的股票）
...
# 2、计算资金曲线的各项评估指标(result) & 盈亏比 & 每年（月、季）的超额收益（excess return）
...
# ===画图
draw_equity_curve_mat(df, date_col='交易日期', data_dict={'策略表现': '净值', '沪深300': '基准净值'},
                      right_axis={'资金使用率': '资金使用率'}, title=pic_title)
# 如果上面的函数不能画图，就用下面的画图
# draw_equity_curve_plotly(df, date_col='交易日期', data_dict={'策略表现': '净值', '沪深300': '基准净值'},
#                   right_axis={'资金使用率': '资金使用率'}, title=pic_title)
# 4、计算资金曲线的盈利（亏损）最大的交易
profit_max, loss_max = get_max_trade(all_stock_data, df, hold_period, stock_num=10)
print(profit_max)
print(loss_max)
