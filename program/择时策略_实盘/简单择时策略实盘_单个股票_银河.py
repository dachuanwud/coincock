...
def main():
    ...
    # ===更新股票持仓信息
    update_one_stock_info(user, stock_info)
    ...
    if sell_stock:
        ...
        # sell_amount = 100
        order_info = user.sell(stock_info['交易代码'], '%.2f' % order_price, '%s' % int(sell_amount))
    ...
    if buy_stock:
        ...
        if buy_amount >= 100:
            # 下单
            order_info = user.buy(stock_info['交易代码'], '%.2f' % order_price, '%s' % int(buy_amount))
